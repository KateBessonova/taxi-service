...
//    }
    ...
    public static void main(String[] args) {
        try (FileInputStream file = new FileInputStream(IMPL_PROPERTIES)) {
            ...
            // importing TripService class
            Class<?> TripService = Class.forName(className);
            // creating instance of TripService class
            Constructor<?> constructor = TripService.getConstructor();
            Object tripService = constructor.newInstance();
            // getting of TripSerive methods
            Method readFromDb = tripService.getClass().getDeclaredMethod("readFromDb", int.class, boolean.class);
            Method readAllFromDb = tripService.getClass().getDeclaredMethod("readAllFromDb", boolean.class);
            Method writeToDb = tripService.getClass().getDeclaredMethod("writeToDb", Customer.class);
            Method updateInDb = tripService.getClass().getDeclaredMethod("updateInDb", Customer.class);
            Method removeFromDb = tripService.getClass().getDeclaredMethod("removeFromDb", int.class);

            // calling TripService methods
            Customer customer1 = new Customer(1, "James", "James@gmail.com");
            Customer customer2 = new Customer(2, "William", "William@gmail.com");
        Driver driver1 = new Driver(1, "Daniel", "daniel@gmail.com", "112233");
        Driver driver2 = new Driver(2, "Thomas", "thomas@gmail.com", "113344");
            ...
            LOGGER.info(writeToDb.invoke(tripService,trip1));
            LOGGER.info(readFromDb.invoke(tripService, 1, false));
            LOGGER.info(updateInDb.invoke(tripService, trip2));
            LOGGER.info(readFromDb.invoke(tripService, 2, false));
            LOGGER.info(removeFromDb.invoke(tripService, 1));
            LOGGER.info(readAllFromDb.invoke(tripService, true));
